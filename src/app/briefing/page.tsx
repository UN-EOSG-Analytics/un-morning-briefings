...
function BriefingContent() {
  ...
  const entriesPerRegion = sortedRegions.reduce((acc, region) => {
    ...
    <div className="min-h-screen bg-white print:bg-white">
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left text-sm py-2 px-3 rounded transition-colors ${
                      isActive
                        ? 'bg-un-blue text-white font-semibold'
      ...
      <div ref={contentRef} className="mx-auto max-w-5xl px-8 sm:px-12 py-12 sm:py-16">
        ...
        <div className="mb-8 sm:mb-10 flex flex-col sm:flex-row items-start sm:items-start justify-between gap-4 sm:gap-0 pb-6 sm:pb-8">
          ...
          <p className="text-xs text-slate-700 tracking-wide text-left sm:text-right">INTERNAL | NOT FOR FURTHER DISTRIBUTION</p>
        </div>

        {/* Title */}
        <h1 className="mb-3 text-center text-4xl font-bold text-un-blue tracking-tight">
        ...
        {sortedRegions.length === 0 ? (
          <p className="text-center text-slate-600">No entries found for this date.</p>
        ) : (
                        ...
                        {entriesByRegionAndCountry[region][country].map((entry, index) => (
                          <div key={entry.id} className="space-y-2.5">
                            ...
                            <p className="text-sm italic text-slate-700 leading-relaxed">
                              {entry.priority === 'sg-attention' ? 'SG Attention' : 'Situational Awareness'}
                              {' | '}
                              {entry.category}
                            </p>
                            ...
                            {/* Source URL */}
                            {entry.sourceUrl && (
                              <p className="text-sm italic text-slate-600 leading-relaxed break-all">
                            ...
                            {/* PU Note */}
                            {entry.puNote && (
                              <div className="text-sm italic text-slate-800 leading-relaxed">
                                <span className="font-bold">PU Note: </span>
                                <span dangerouslySetInnerHTML={{ __html: entry.puNote }} />
                              </div>
                            )}

                            {/* Separator between entries */}
                            {index < entriesByRegionAndCountry[region][country].length - 1 && (
